...
async search (req,res) {        
    const query = url.parse(req.url, true).pathname.split("/")[3]
    const index = url.parse(req.url, true).pathname.split("/")[2]

    console.log(url.parse(req.url, true).pathname.split("/"))
    var data = JSON.stringify([
      ...
      var config = {
        ...
        headers: { 
          ...
          'medium-frontend-path': '/tag/'+`${query}`, 
